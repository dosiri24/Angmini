...
### 4.1 서버 응답 파싱
- [x] **IMPL**: 서버 응답에서 일정 데이터 추출 로직
- [x] **IMPL**: 파싱된 데이터로 달력 업데이트
- [x] **IMPL**: 파싱 실패 시 에러 핸들링

### 4.2 로컬 캐시
- [x] **IMPL**: 최근 대화 내역 저장 (앱 재시작 시 복원)
- [x] **IMPL**: 일정 데이터 캐시 (오프라인 조회용)
- [x] **IMPL**: localStorage 사용

### 4.3 설정 화면
- [x] **IMPL**: 설정 화면 모달 구현
- [x] **IMPL**: Bot Token, Channel ID, Bot User ID 입력/저장
- [x] **IMPL**: 미설정 시 설정 버튼 강조 표시

### 4.4 창 속성 마무리
- [x] **IMPL**: "항상 위에" 옵션 제공 (설정 화면에서 토글)
- [ ] **IMPL**: 시스템 트레이 상주 (선택, 미구현)
- [ ] **IMPL**: 타이틀바 최종 조정 (선택, 기본값 유지)

### 4.5 캐릭터 이미지 교체 준비
- [ ] **IMPL**: 플레이스홀더 → 실제 GIF/이미지 교체 가능한 구조 (사용자가 직접 추가 예정)
- [ ] **IMPL**: 이미지 네이밍 규칙: character_idle.gif 등

### 4.6 문서화
- [x] **DOCS**: README.md - 설치 및 실행 방법
- [x] **DOCS**: 빌드 및 배포 가이드
...
| 4. 통합/마무리 | ✅ 완료 | 2025-11-26 | 2025-11-26 |
