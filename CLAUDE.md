...
### 🚨 1. 순수 LLM 기반 원칙 (No Keyword Parsing) - 최우선 규칙

> **이 규칙은 프로젝트의 핵심 철학입니다. 절대 위반하지 마세요.**

#### 핵심 원칙
- **자연어 키워드 파싱/정규식 라우팅 절대 금지**: 모든 자연어 처리는 LLM이 담당
- **도구 사용(Tool Use) 패턴**: LLM이 의도를 파악하고 적절한 도구를 호출
- **Tool은 구조화된 데이터만 처리**: ISO 형식 (YYYY-MM-DD, HH:MM) 등
- **모델 이분법 (임의 변경 절대 금지)**:
  - `gemini-flash-latest`: 빠른 응답용 (일반 대화, 단순 Tool 호출)
  - `gemini-3.0-pro`: 복잡한 추론용 (다단계 분석, 어려운 판단)
  - ⚠️ **이 모델명은 프로젝트 표준입니다. 임의로 다른 모델로 변경하지 마세요.**
- 슬래시 커맨드(`/help` 등)만 예외적으로 직접 라우팅 허용

#### 올바른 아키텍처
```
사용자: "내일 오후 3시에 팀 미팅 추가해줘"
    ↓
LLM: 자연어 이해 + 구조화 변환  ← ✅ LLM이 100% 담당
    ↓
LLM: add_schedule(
    title="팀 미팅",
    date="2025-11-27",      ← ISO 형식으로 변환 완료
    start_time="15:00"       ← 24시간제로 변환 완료
)
    ↓
Tool: 검증 → DB 저장  ← 파싱 로직 없음, 구조화 데이터만 처리
```
...
1. **🚨 순수 LLM 원칙**: 자연어 파싱 금지 (1번 규칙)
2. **안전성 (Security)**: 데이터 손실 방지, 시크릿 유출 방지 (`.env` 읽기 금지)
3. **무결성 (Integrity)**: TDD 통과 및 문서와의 일치
4. **간결성 (Simplicity)**: YAGNI 및 오버엔지니어링 방지
5. **명확성 (Clarity)**: 가독성 높은 코드와 문서
...
| 항목 | 금지 사항 | 심각도 |
|------|----------|--------|
| **🚨 자연어 처리** | **키워드 파싱, 정규식 라우팅, if-else 의도 분류, parse_natural_* 함수** | **치명적** |
| **🚨 Tool 설계** | **Tool 내부에서 자연어 변환 (LLM이 해야 함)** | **치명적** |
| **🚨 모델명** | **gemini-flash-latest / gemini-3.0-pro 외 다른 모델로 임의 변경** | **치명적** |
| 기능 | 요청하지 않은 기능 추가, 미래를 대비한 코드 | 높음 |
| 코드 | 모킹 남용, 테스트용 print 방치, 하드코딩된 시크릿 | 높음 |
| 컨텍스트 | **파일 내용을 읽지 않고 추측하기(Guessing)** | 높음 |
| 프로세스 | **계획(`<plan>`) 및 테스트 없이 코드 작성 시작** | 높음 |
| 세션 | **긴 세션에서 컨텍스트 정리 없이 작업 지속 (Context Rot)** | 중간 |
| 소통 | "아까 말했듯이"라며 채팅 기억에 의존하기 | 중간 |
| 문서 | 코드는 수정했으나 문서는 방치하는 행위 | 중간 |
