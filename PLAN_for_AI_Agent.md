...
- [ ] **3.4: Apple 시스템 도구 구현 (Apple MCP 연동)**
    ...
    - [ ] 3.4.3: **AppleTool 운영 품질 다듬기**
        - [ ] 3.4.3.1: `AppleMCPManager`/`STDIOCommunicator` 재시작·대기 로직 점검 및 로그 정비
        - [ ] 3.4.3.2: AppleTool 타임아웃·재시도 기본값 검토와 성능 메트릭 정리
        - [ ] 3.4.3.3: AppleTool 보안 검사 규칙(금지 패턴/텍스트 길이) 검토와 테스트 추가
        - [ ] 3.4.3.4: 동기 실행 기반 장애 시나리오 점검 및 수동 부하 테스트 체크리스트 작성
    - [ ] 3.4.4: **통합 및 검증**
        - [x] 3.4.4.1: `ToolManager` 기본 등록 확인 및 실패 시 사용자 알림 메시지 정의
        - [ ] 3.4.4.2: CLI/Discord 인터페이스에서 Apple 명령 수동 점검 시나리오 문서화
        - [ ] 3.4.4.3: AppleTool 통합 테스트 스켈레톤 정비 (macOS 권한 가이드 포함)
        - [ ] 3.4.4.4: 간단 사용 가이드/FAQ 작성 (`docs/APPLE_TOOL_GUIDE.md`)
